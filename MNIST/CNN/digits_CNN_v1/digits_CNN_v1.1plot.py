...
def plot_init():
    plot_x, plot_y = [0], [0]
    fig, ax = plt.subplots()
    line, = ax.plot(plot_x, plot_y, label = "Accuracy")
    ...
    return plot_x, plot_y, fig, ax, line, label

def plot_update(ax, line, plot_x, plot_y, progress, batch, epoch):
    ax.set_title(f"Progress: {round(progress * 100, 2)}%")
    line.set_xdata(plot_x)
    line.set_ydata(plot_y)
    ax.set(xlim = (0, (batches * epoch + batch) * batch_size), ylim = (0, 100))
    plt.draw()
    plt.pause(0.001)
...
def train(x_train, y_train):
    ...
    train_correct = 0
    batches_completed = 0
    plot_x, plot_y, fig, ax, line, label = plot_init()
    ...
    for epoch in range(epochs):
        ...
        for batch, (image, target) in enumerate(zip(batched_x_train, batched_y_train)): 
            ...
            train_correct += np.sum((np.argmax(x[-1], axis = 1) == target).astype(int))

            if (batch + 1) % plot_batches == 0: 
                progress = (epoch / epochs) + (1.0 / epochs) * (batch / batches)
                plot_y = plot_y + [(100.0 * train_correct) / (batch_size * plot_batches)]
                plot_x = plot_x + [(batches * epoch + batch) * batch_size]

                plot_update(ax, line, plot_x, plot_y, progress, batch, epoch)
                train_correct = 0
...
def test():
    ...
    x_train, y_train = training_data.values[:, 1:] / 255, training_data.values[:, 0] # normalize
    ...
    # evaluation
    x_test_prop, _0, _1 = forward(w, b, x_test, False)
